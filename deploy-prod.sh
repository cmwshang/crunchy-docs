...
REGION="us-east-2"
BUCKET="www.crunchydata.com"
DEPLOY_DIR=".deploy"

#=== Google Tag Manager ===#
GTM_HEAD="<!-- Google Tag Manager --><script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl+ '&gtm_auth=8cS6EeyGOAu9GjNlblffBw&gtm_preview=env-2&gtm_cookies_win=x';f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-NXJR9MD');</script><!-- End Google Tag Manager -->"
GTM_BODY='<!-- Google Tag Manager (noscript) --><noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-NXJR9MD&gtm_auth=8cS6EeyGOAu9GjNlblffBw&gtm_preview=env-2&gtm_cookies_win=x" height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript><!-- End Google Tag Manager (noscript) -->'
...
for filename in $(find $DEPLOY_DIR -type f -iname '*.redirect'); do
  ...
  # Move it
  mv $original $clean
  aws s3 sync $DEPLOY_DIR s3://$BUCKET --include '$clean' --exclude '*.*' --no-guess-mime-type --content-type 'text/html' --website-redirect $(cat $clean) --size-only --region $REGION --profile 'packagingadmin'
done
...
aws s3 sync $DEPLOY_DIR s3://$BUCKET --include '*[!.*]' --exclude '*.*' --no-guess-mime-type --content-type 'text/html' --size-only --region $REGION --profile 'packagingadmin'

# Sync all remaining files which have an extension
echo "SYNCING FILES WITH AN EXTENSION TO BUCKET '${BUCKET}' REGION '${REGION}'"
aws s3 sync --delete $DEPLOY_DIR s3://$BUCKET --include '*.*' --size-only --region $REGION --profile 'packagingadmin'
